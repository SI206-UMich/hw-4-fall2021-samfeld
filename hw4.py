...
class Stall:
    ...
    def has_item(self, name, quantity):
        ...
        if self.name in self.invenotry:
            if self.invenotry[self.name]>=quantity:
                return True
            else:
                return False
        else:
    ...
    def stock_up(self, name, quantity):
        ...
        #self.quantity=quantity
        if self.name in self.invenotry:
            self.invenotry[self.name]+=quantity
        else:
            self.invenotry[self.name]=quantity
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_validate_order(self):
        ...
        self.assertEqual(self.f1.validate_order(self.c2, self.s2, "Taco", 12), "Don't have enough money for that :( Please reload more money!")
